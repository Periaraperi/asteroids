...
Texture::Texture(uint32_t width, uint32_t height)
    :_width{width}, _height{height}, _buffer{std::vector<uint8_t>(4*width*height, 255)}
{
    PERIA_LOG("Arbitrary Texture Ctor()");
    GL_CALL(glGenTextures(1, &_tex));
    bind();
...
void Texture::write_sub_texture_color(uint32_t xoffset, uint32_t yoffset,
                                uint32_t width, uint32_t height)
{
    std::vector<uint8_t> data(width*height, 150);
    bind();
    GL_CALL(glPixelStorei(GL_UNPACK_ALIGNMENT, 1));
    GL_CALL(glTexSubImage2D(GL_TEXTURE_2D, 0,
                            xoffset, yoffset, 
